...
| CodeRabbit | `cr review --prompt-only -t uncommitted` | Requires `CODERABBIT_API_KEY` env var or `cr auth login`. Installed via `curl -fsSL https://cli.coderabbit.ai/install.sh \| sh`. |
