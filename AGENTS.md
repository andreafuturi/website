...
- **Deno dependencies are URL imports** — pulled from `esm.sh`, `deno.land/std`, and `cdn.skypack.dev` at runtime, not from npm. See `server/importMap.json`. These three domains must be added to the Copilot agent firewall allowlist (repo **Settings → Copilot → Coding agent → Firewall**) so the agent can download them during setup and when importing new modules.
