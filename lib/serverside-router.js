    ...
    const scan = async dir => {
      ...
      try {
        for await (const entry of Deno.readDir(dir)) {
          ...
          // Add more robust error handling and path normalization
          try {
            if (entry.isDirectory) {
              ...
              const folderMatchPath = new URL(`file://${fullPath}/${entry.name}${extension}`).pathname;
              try {
                ...
                console.error(`⚠️ Folder pattern check error: ${error.message}`);
                await scan(fullPath);
              }
              continue;
            }
            ...
            const importPath = new URL(`file://${fullPath}`).pathname;
...
export function createRouter({
  ...
  const pageManager = createRouteManager(pagesDirectory, ".jsx", ["main.jsx", "index.jsx"], ["dist", "portfolio", "components", "images"]);
      ...
      if (path === "debug-paths") {
        return new Response(
          JSON.stringify({
            ...
            apiEndpointsPath,
            basePaths: {
              api: apiEndpointsPath,
              pages: pagesDirectory,
            },
            env: {
              isDev: isDevelopmentMode,
              denoDeployment: Deno.env.get("DENO_DEPLOYMENT_ID") ? true : false,
            },
          }),
          {
