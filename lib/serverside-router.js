        ...
        if (entry.isDirectory) {
          ...
          try {
            ...
            if (stat.isFile && !excludeFiles.includes(entry.name + extension)) {
              ...
              // Use import.meta.resolve for more reliable imports
              const moduleUrl = new URL(folderMatchPath, import.meta.url).href;
              console.log("📦 Importing module from:", moduleUrl);
              const module = await import(moduleUrl);
              routes.set(routePath, module.default);
            }
          } catch (error) {
            console.log("❌ No folder match, scanning directory:", fullPath, error);
            await scan(fullPath);
          }
          continue;
        }
