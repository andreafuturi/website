...
function createRouteManager(basePath, extension, excludeFiles = [], excludeDirs = []) {
  ...
  async function scanRoutes() {
    const scan = async dir => {
      ...
      for await (const entry of Deno.readDir(dir)) {
        ...
        if (entry.isDirectory) {
          ...
          try {
            ...
            if (stat.isFile && !excludeFiles.includes(entry.name + extension)) {
              ...
              console.log(`✅ Found valid route: ${routePath}`);
              const module = await import(folderMatchPath);
              routes.set(routePath, module.default);
            }
          } catch (error) {
            console.error(`❌ Error checking folder/file pattern: ${error.message}`);
        ...
        if (!entry.isFile || !entry.name.endsWith(extension) || excludeFiles.includes(entry.name)) {
  ...
  /** @param {Record<string, unknown>} routesObject */
  ...
  /** Match explicit or scanned routes in the Map 🎯 */
  function matchFromMap(normalizedPath) {
    const direct = routes.get(normalizedPath);
    ...
    for (const [routePath, routeHandler] of routes.entries()) {
      if (isDynamicRoute(routePath)) {
        ...
        if (params) return { handler: routeHandler, params };
      }
    }
    return null;
  }
  ...
  async function getHandler(path) {
    ...
    // Fallback to existing dynamic route resolution
    if (routes.size > 0) {
      const fromMap = matchFromMap(normalizedPath);
      if (fromMap) return fromMap;
    }

    // Dev: file-based imports only (no full tree scan)
    if (isDev) {
    ...
    return matchFromMap(normalizedPath);
  }

  return { getHandler, initializeRoutes };
}
...
export function createRouter({
  ...
  handleError,
  handleStaticFiles,
  routes = {}, // Add routes parameter
} = {}) {
  const apiManager = createRouteManager(apiEndpointsPath, ".js");
  const pageManager = createRouteManager(pagesDirectory, ".jsx", ["main.jsx", "index.jsx"], ["dist", "portfolio", "components", "images", "teleporty"]);
  ...
  return async function handleRequest(req) {
    ...
    try {
      ...
      if (path.startsWith("api/")) {
        ...
        const routeMatch = await apiManager.getHandler(apiPath);

        if (!routeMatch) {
          console.error(`❌ API route not found: ${apiPath}`);
          throw new Error(`API route not found: ${apiPath}. Add it under routingConfig.routes or place a default export in ${apiEndpointsPath}/${apiPath}.js`);
        }

        const { handler: apiHandler, params } = routeMatch;
        if (!apiHandler || typeof apiHandler !== "function") {
          console.error(`❌ Invalid API handler for route: ${apiPath}`, routeMatch);
          throw new Error(`Invalid API handler: export a default async function from ${apiEndpointsPath}/${apiPath}.js (got ${typeof apiHandler})`);
        }

        const result = await apiHandler(body ?? {}, req, params);
      ...
      // Non-JSX paths with extensions are static assets (Singularity routing model)
      if (handleStaticFiles && /\.[a-z0-9]+$/i.test(path.split("/").pop() || "") && !path.endsWith(".jsx")) {
      ...
      if (body === "onlyRoute") {
        return new Response(await renderFunction(await Page({ params, originRequest: req, ...body })), {
