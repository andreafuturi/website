  ...
  async function scanRoutes() {
    const scan = async dir => {
      ...
      for await (const entry of Deno.readDir(dir)) {
        ...
        if (entry.isDirectory) {
          ...
          try {
            ...
            if (stat.isFile && !excludeFiles.includes(entry.name + extension)) {
              ...
              console.log("✅ Found folder match:", folderMatchPath);

              // Fix: Use the correct path relative to client directory
              const moduleUrl = `../client/${routePath}/${entry.name}${extension}`;
              console.log("📦 Importing module from:", moduleUrl);
              const module = await import(moduleUrl);
              routes.set(routePath, module.default);
            }
          } catch (error) {
            console.log("❌ No folder match, scanning directory:", fullPath);
            await scan(fullPath);
          }
          continue;
        }
        ...
        // Fix: Use the correct path relative to client directory
        const moduleUrl = `../client/${routePath}${extension}`;
        console.log("📦 Importing module from:", moduleUrl);
        const module = await import(moduleUrl);
        routes.set(routePath, module.default);
      }
